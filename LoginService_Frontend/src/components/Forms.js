...
class Forms extends React.Component {
  ...
  validateForm() {
    ...
    if (!flag) {
      ...
      userObject.password["pwd1"] = this.state.fields.hpassword;
    ...
    if (flag) {
      ...
      emailIDObject.password["pwd1"] = this.state.fields.hpassword;
      if (typeof fields["username"] !== "undefined") {
  ...
  axiosReponse(url, obj) {
    //console.log(`axiosReposne Called with url: ${url} and Object :${obj.password.pwd1}`)
    Axios.post(url, obj)
      .then(
        res => {
      ...
      if (res.data.loginStatusMessage === "Authenticated") {
       // alert(res.data.loginStatusMessage)
        ...
        window.location.assign(`http://localhost:8002/#/confirmuser/${uid}`);
  ...
  user(arg) {
    ...
    if (this.validateForm()) {
      ...
      let userObject = { password: {} }
      let emailIDObject = { emailID: '', password: { pwd: '' } }
      
      userObject["userID"] = "";
      userObject["password"]["pwd1"] = "";
